...
def create_dataset(dataset_opt, phase, stage2_file=None):
    """
    根据 dataset_opt['name'] 自动选择 MRI / CT / Mayo CT 数据集
    """
    ...
    if 'mayo' in dataset_name:
        ...
        dataset = MayoCTDataset(
            ...
            train_batches=dataset_opt.get('train_batches', ['train']),
            val_batches=dataset_opt.get('val_batches', ['val']),
            ...
            teacher_n2n_epoch=dataset_opt.get('teacher_n2n_epoch', 70),
            num_slices=dataset_opt.get('num_slices', 100),
            # 指定患者
            patient_ids=dataset_opt.get('patient_ids', None),
        )

        logger.info(f'Mayo CT dataset [{dataset_opt.get("name","mayo_ct")}] is created. Size: {len(dataset)}')
        return dataset
